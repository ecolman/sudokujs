...
export default class Menu {
  ...
  constructor(paper, board) {
    ...
    let sudokuCellValues = [1, 2, 3, 4, 5, 6, 7, 8, 9];
    this.menuCellsValues = [];

    // push randomly selected value to cells
    for (let i = 0; i < uiConstants.menuCellsToShow.length; i++) {
  ...
  createBgNums() {
    for (let i = 0; i < uiConstants.menuCellsToShow.length; i++) {
      let rect = this.paper.getById(uiConstants.menuCellsToShow[i]);   // get the holder rectang
      let cellText = this.paper.text(rect.attr('x') + (uiConstants.sizes.cell.width / 2), rect.attr('y') + (uiConstants.sizes.cell.height / 1.45),
        this.menuCellsValues[i]);  // create numeral

      cellText.node.id = 'menuCell' + i;  // set cell node id

      // set class and data-menu
      $(cellText.node).attr({
        class: uiConstants.classes.textCell,
  ...
  createHomeMenu() {
    // create rectangle around menu to make everything opaque
    let bgRect = this.paper.rect(3, 50, 540, 450).attr({ fill: '#E0F8E0', 'stroke-width': 0 });
    ...
    // easy option
    let easyOption = this.paper.text(275, 125, 'Easy').attr({ fill: 'green', 'letter-spacing': 15 });
    $(easyOption.node).attr({ class: uiConstants.classes.menu, 'data-type': uiConstants.menuOptionType.easy, 'data-menu': true, 'data-main': true });   // set class and data attrs

    // medium option
    let mediumOption = this.paper.text(275, 175, 'Medium').attr({ fill: 'green', 'letter-spacing': 13 });
    $(mediumOption.node).attr({ class: uiConstants.classes.menu, 'data-type': uiConstants.menuOptionType.medium, 'data-menu': true, 'data-main': true });   // set class and data attrs

    // hard option
    var hardOption = this.paper.text(275, 225, 'Hard').attr({ fill: 'green', 'letter-spacing': 15 })
    $(hardOption.node).attr({ class: uiConstants.classes.menu, 'data-type': uiConstants.menuOptionType.hard, 'data-menu': true, 'data-main': true });   // set class and data attrs

    // expert option
    var expertOption = this.paper.text(275, 275, 'Expert').attr({ fill: 'green', 'letter-spacing': 12 })
    $(expertOption.node).attr({ class: uiConstants.classes.menu, 'data-type': uiConstants.menuOptionType.expert, 'data-menu': true, 'data-main': true });   // set class and data attrs

    // load option
    var loadOption = this.paper.text(275, 385, 'Load Last Game').attr({ fill: '#562E60', 'letter-spacing': 2 })
    $(loadOption.node).attr({ class: uiConstants.classes.smallMenu, 'data-type': uiConstants.menuOptionType.load, 'data-menu': true, 'data-main': true });   // set class and data attrs

    // resume option
    var resumeOption = this.paper.text(275, 340, 'Resume Game').attr({ fill: '#562E60', 'letter-spacing': 5 })
    $(resumeOption.node).attr({ class: uiConstants.classes.smallMenu, 'data-type': uiConstants.menuOptionType.resume, 'data-menu': true, 'data-main': true }).hide();   // set class and data attrs, then hide

    // options...option
    var optionsMenuOption = this.paper.text(275, 430, 'Options').attr({ fill: '#562E60', 'letter-spacing': 7 })
    $(optionsMenuOption.node).attr({ class: uiConstants.classes.smallMenu, 'data-type': uiConstants.menuOptionType.options, 'data-raphael-id': optionsMenuOption.id, 'data-menu': true,
    ...
    $('[data-menu=true][data-main=true]').bind(this.clickEventType, function (event) {
        var optionType = $(this).attr('data-type');

        switch (optionType) {
          case uiConstants.menuOptionType.easy:
            this.board.difficulty = uiConstants.boardDifficulty.easy;
            this.boardView(uiConstants.boardLoadType.fresh);
            break;

          case uiConstants.menuOptionType.medium:
            this.board.difficulty = uiConstants.boardDifficulty.medium;
            this.boardView(uiConstants.boardLoadType.fresh);
            break;

          case uiConstants.menuOptionType.hard:
            // sudoku.difficulty = uiConstants.boardDifficulty.hard;
            // menu.boardView(uiConstants.boardLoadType.fresh);
            break;

          case uiConstants.menuOptionType.expert:
            // sudoku.difficulty = uiConstants.boardDifficulty.expert;
            // menu.boardView(uiConstants.boardLoadType.fresh);
            break;

          case uiConstants.menuOptionType.load:
            //menu.boardView(uiConstants.boardLoadType.load);
            break;

          case uiConstants.menuOptionType.resume:
            //menu.boardView(uiConstants.boardLoadType.resume);
            break;

          case uiConstants.menuOptionType.options:
            //menu.optionsView();
            break;
  ...
  boardView(loadType) {
    ...
    switch (loadType) {
      case uiConstants.boardLoadType.fresh:
        // based on selected difficulty, cull # of cells or pick from stored JSON boards
        switch (uiConstants.sudoku.difficulty) {
          case uiConstants.boardDifficulty.easy:
              sudoku.cull(42);
              sudoku.guaranteeUniqueness(42);
              break;

          case uiConstants.boardDifficulty.medium:
              sudoku.cull(50);
              sudoku.guaranteeUniqueness(50);
              break;

          case uiConstants.boardDifficulty.hard:
              sudoku.loadBoard(250, 350);
              break;

          case uiConstants.boardDifficulty.expert:
...
//         $(easyOption.node).attr({ class: uiConstants.menuClass, 'data-type': menuOptionType.easy, 'data-menu': true, 'data-main': true });   // set class and data attrs

//         // medium option
//         var mediumOption = this.paper.text(275, 175, 'Medium').attr({ fill: getDifficultyColor(boardDifficulty.medium), 'letter-spacing': 13 });
//         $(mediumOption.node).attr({ class: uiConstants.menuClass, 'data-type': menuOptionType.medium, 'data-menu': true, 'data-main': true });   // set class and data attrs

//         // hard option
//         var hardOption = this.paper.text(275, 225, 'Hard').attr({ fill: getDifficultyColor(boardDifficulty.hard), 'letter-spacing': 15 })
//         $(hardOption.node).attr({ class: uiConstants.menuClass, 'data-type': menuOptionType.hard, 'data-menu': true, 'data-main': true });   // set class and data attrs

//         // expert option
//         var expertOption = this.paper.text(275, 275, 'Expert').attr({ fill: getDifficultyColor(boardDifficulty.expert), 'letter-spacing': 12 })
//         $(expertOption.node).attr({ class: uiConstants.menuClass, 'data-type': menuOptionType.expert, 'data-menu': true, 'data-main': true });   // set class and data attrs
