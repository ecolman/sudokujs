...
function Events(props) {
  ...
  useEffect(() => {
    latestProps.current = { baseBoard, isSolved, selectedCell };
  });
  ...
  function handleKeyDown(event) {
    let { baseBoard, selectedCell, isSolved } = latestProps.current;

    const key = event.charCode || event.keyCode || 0;

    if ((key >= 49 && key <= 57 || key >= 97 && key <= 105) && !isSolved) {
